...
class AudioDataProviderElement(Element):
    ...
    def update(self, rs: RenderState):
        ...
        if data is not None and sound and pygame.mixer.get_busy():
            idx = max(0, int((rs.time + audio_vis_offset_ms / 1000.0) * samplerate))
        ...
        flux = float(np.sum(np.maximum(0.0, rs.fft - rs.prev_fft)))
        rs.prev_fft[:] = rs.fft
        rs.flux_history.append(flux)
        if len(rs.flux_history) > 40:
            rs.flux_history.pop(0)
        avg_flux = float(np.mean(rs.flux_history[:-1])) if len(rs.flux_history) > 4 else flux
        flux_beat = (
            avg_flux > 1e-6
            and flux > max(0.02, avg_flux * 1.5)
        )
        rms_avg = (
            np.mean(rs.rms_history[:-1])
            if len(rs.rms_history) > 4
            else rs.rms
        )
        rms_beat = (
            rs.rms > max(0.02, rms_avg * 1.3)
        )

        rs.energy = rs.energy * 0.82 + float(np.mean(rs.fft)) * 0.18
        rs.is_beat = flux_beat or rms_beat
        if rs.is_beat:
            rs.smooth_beat = min(
                1.0,
                rs.smooth_beat * 0.4 + rs.rms_smooth * 4.0
            )
        else:
            rs.smooth_beat *= 0.90
        ...
        rs.peak_bar_index = int(np.argmax(rs.smooth_bars))

        if rs.is_beat:
            amt = 4.0 + rs.rms_smooth * 30.0
...
class ImageElement(Element):
    def render(self, surface, rs):
        ...
        rad = (
            int(min(WIDTH, HEIGHT) * 0.145)
            + int(rs.rms_smooth * 120)
        )
        glow_rad = rad + 24 + int(rs.rms_smooth * 180)
